...
export class ChatsService {
  ...
  async createChat(dto: CreateChatDto) {
    const participantIds = Array.from(new Set(dto.participantIds || [])).map(id => id.trim()).filter(Boolean);
    if (participantIds.length < 2) {
      throw new BadRequestException('chat heeft minstens twee deelnemers nodig');
    }
